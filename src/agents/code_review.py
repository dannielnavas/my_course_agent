...
class SecurityReview(BaseModel):
    vulnerabilities: list[str] = Field(
        description="The vulnerabilities in the code", default=None)
    riskLevel: str = Field(
        description="The risk level of the vulnerabilities", default=None)
    suggestions: list[str] = Field(
        description="The suggestions to fixing the vulnerabilities", default=None)


class MaintainabilityReview(BaseModel):
    concerns: list[str] = Field(
        description="The concerns about the code", default=None)
    qualityScore: int = Field(
        description="The quality score of the code from 1 to 10", default=None, ge=1, le=10)
    recommendations: list[str] = Field(
        description="The recommendations for improving the code", default=None)
...
def maintainability_review(state: State):
    code = state["code"]
    messages = [
        ("system", "You are an expert in code quality. Focus on structure, readability and adherence to best practices."),
        ("user", f"Review this code: {code}")
    ]
...
def aggregator(state: State):
    ...
    messages = [
        ...
        ("user",
         f"Systhesize these code review results into a concise summary with key actions: Security Review: {security_review}, Maintainability Review: {maintainability_review}")
    ]
