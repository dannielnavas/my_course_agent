...
def generator_node(state: State):
    ...
    if feedback:
        msg = llm.invoke(
            f"Write a joke about {topic} but take into account the feedback: {feedback} respond in Spanish")
    else:
...
def evaluator_node(state: State):
    ...
    schema = ll_evaluator.invoke(
        f"Grade the joke {joke} with the following prompt: {SYSTEM_PROMPT}")
